...
def execute_query(conn, query, params, transform=None, unique=False):
    ...
    Args:
        conn:       engine or connection object. (if connection is passed, 
                    transaction can be rolled back by the caller)
        query:      sql query in string format
        params:     string parameters for query string
        transform:  a callable that accepts a mutable, subsriptable object 
                    and returns a modified object of the same type
        unique:     boolean. Returns a single object. 
    Returns:
        List of dicts if unique=False, else a single dict, and an http 
        status code. In case of exception, returns empty dict and 500 code.
    ...
    try:
        for row in conn.execute(query, params):
            row = dict(row)
            if not checked:
                datetime_columns = set([k for k in row.keys() if type(row[k]) == datetime])
                checked = 1
